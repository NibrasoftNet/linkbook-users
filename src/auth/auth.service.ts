...
import { JwtPayloadType, JwtRefreshPayloadType, NullableType } from '@NibrasoftNet/linkbook-commons';
...
export class AuthService implements OnModuleInit {
	...
	onModuleInit() {
		this.sharedService =
			this.client.getService<SharedServiceClient>('SharedService');
	}
	...
	async registerUser(
		...
		const emailResult = await firstValueFrom(
			this.sharedService.confirmEmail({
				otp: data.otp,
	...
	async resendOtp(
		...
		const otpRes = otp
			? await this.otpService.update(otp.id, otp, data)
			: await this.otpService.create(data);
		const emailResult = await firstValueFrom(
			this.sharedService.confirmEmail({
				otp: data.otp,
				expiryDate: data.expiryDate,
	...
	async forgotPassword(email: string): Promise<AuthRegisterResponseDto> {
		...
		return this.resendOtp({ userId: user.id });
	}

	async confirmOTP(confirmEmailDto: ConfirmEmailDto) {
		const userOtp = await this.otpService.findOneOrFail({
			userId: confirmEmailDto.userId,
		});
